...
RETURNING position_id, ref_position_id, trading_acct, client_id, product_code, exchange_code, instrument_code, position_side, position_status, position_type, settlement_status, calc_type, open_qty, open_px, closed_qty, closed_px, trading_curr, settle_curr, mark_price, mark_value, exchange_rate, unrealized_pl, realized_pl, mm_and_cash, collateral_haircut_ratio, collateral_haircut, open_date, closed_date, cost_to_close, trade_date, trading_session_id, underlying_code, open_close, put_or_call, contract_size, created_at, updated_at
`
...
func (q *Queries) GetPositionForUpdate(ctx context.Context, positionID string) (Position, error) {
	row := q.db.QueryRow(ctx, getPositionForUpdate, positionID)
	var i Position
	err := row.Scan(
		&i.PositionID,
		&i.RefPositionID,
